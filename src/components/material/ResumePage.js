          ...
          <div className="resume-flex-container">
            <div className="resume-left-pane">
              {experiences.map((exp, idx) => {
                ...
                return <Experiences content={content} key={idx} />;
              })}
            </div>
...
function generateStars(str) {
  let full = Array.apply(null, { length: str }).map((v, idx) => (
    <FontAwesome className="fa fa-star" name="star" key={idx} />
  ));
  let empty = Array.apply(null, { length: 5 - str }).map((v, idx) => (
    <FontAwesome className="fa fa-star-o" name="star" key={-idx - 1} />
  ));
...
export const ResumeSkills = props => {
  return (
    <SideBlock className="resume-skills-block">
      <p className="resume-skills-header">
